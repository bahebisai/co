...
import com.zoomtech.emm.features.progress.CircleProgress;
...
import com.zoomtech.emm.view.activity.AppUpdateActivity;
import com.zoomtech.emm.view.activity.FileUpdateActivity;
import com.zoomtech.emm.view.activity.MessageActivity;
import com.zoomtech.emm.view.adapter.LauncherAdapter;
import com.zoomtech.emm.view.adapter.WorkBenchAdapter;
        ...
        if (theTang.getUsageStats()) {
            if (getActivity() != null) {
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        wifi_count.setText(ConvertUtils.convertTraffic(DataFlowStatsHelper.getAllMonthWifi()/*dataFlowStatsHelper.getAllRxBytesWifi()+dataFlowStatsHelper.getAllTxBytesWifi()*/));
                        data_count.setText(ConvertUtils.convertTraffic(DataFlowStatsHelper.getAllMonthMobile(TheTang.getSingleInstance().getContext(), null)));
                    }
                });
            }
        } else {
