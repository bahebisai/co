...
import com.zoomtech.emm.view.photoview.adapter.ImagePickerAdapter;
...
public class ImagePickerActivity extends AppCompatActivity implements View.OnClickListener, ImagePickerAdapter.ChangerNumberListener, ImagePickerAdapter.RecyClerItemClickListener {
		...
		switch (requestCode) {
			case REQUEST_CODE_READ_EXTERNAL_STORAGE:
				if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
					...
					toast(getResources().getString(R.string.permission_refuse));
				}
				break;
		}
	}
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.tv_complete:
				...
				//	toast("至少选择一张图片");
					toast(getResources().getString(R.string.pick_one_photo));
					return;
				}
	...
	public void onNumberChanged() {
		if (mAdapter.checkNumber == 0) {
			...
			mTvComplete.setText( getResources().getString(R.string.finish) + "("+mAdapter.checkNumber+")");
		}
	}
