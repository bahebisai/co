package com.zoomtech.emm.features.luban;
...
public class Luban implements Handler.Callback {
  ...
  private File getImageCacheFile(Context context, String suffix) {
    ...
    String cacheBuilder = mTargetDir + "/" +
        ...
        //(TextUtils.isEmpty(suffix) ? ".png" : suffix);
          ...
          @Override public void run() {
            try {
              ...
              File result = Checker.isNeedCompress(mLeastCompressSize, path) ?
                  new Engine(path, getImageCacheFile(context, Checker.checkSuffix(path))).compress(10) :
                  new File(path);
  ...
  @WorkerThread private File get(String path, Context context) throws IOException {
    return new Engine(path, getImageCacheFile(context, Checker.checkSuffix(path))).compress(10);
  }
  ...
  @WorkerThread private List<File> get(Context context) throws IOException {
    ...
    while (iterator.hasNext()) {
      ...
      //if (Checker.isImage( path )) {
        ...
        while (originFile.length() > 1 * 1024 * 1024) {

          originFile = new Engine( path,
                  getImageCacheFile( context, Checker.checkSuffix( getImageCacheDir( context ).getAbsolutePath() ) ) ).compress( ++count );
